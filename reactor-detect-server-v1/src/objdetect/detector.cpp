...
Detector::Detector(const std::string& weight, const torch::DeviceType& device, float conf_threshold, float iou_threshold) :
	_device(device),
...
/*	inference.	*/
std::vector<std::tuple<cv::Rect, float, int>> Detector::Forward(const cv::Mat& img)
{
	...
	auto tensor_img = torch::from_blob(img_input.data, { 1, img_input.rows, img_input.cols, img_input.channels() });
	tensor_img = tensor_img.permute({ 0, 3, 1, 2 }).contiguous();	// BHWC -> BCHW (Batch, Channel, Height, Width)

	std::vector<torch::jit::IValue> inputs;
	inputs.emplace_back(tensor_img);
	// inference.
	torch::jit::IValue outputs = _module.forward(inputs);
	auto detections = outputs.toTuple()->elements()[0].toTensor();

	// result: n * 7
	// batch index(0), top-left x/y (1,2), bottom-right x/y (3,4), score(5), class id(6)
	auto result = PostProcessing(detections, _conf_threshold, _iou_threshold);
	...
	const auto& result_datas = result.index_select(0, idx_mask_index).slice(1, 1, 7);

	// use accessor to access tensor elements efficiently.
	const auto& demo_data = result_datas.accessor<float, 2>();

	// remap to original image and list bounding boxes for debugging purpose.
	std::vector<std::tuple<cv::Rect, float, int>> data_vec;
...
inline torch::Tensor Detector::GetBoundingBoxIoU(const torch::Tensor& box1, const torch::Tensor& box2)
{
...
torch::Tensor Detector::PostProcessing(const torch::Tensor& detections, float conf_threshold, float iou_threshold)
{
...
/*	load class names	*/
void Detector::LoadClassNames(const std::string& path)
{
	// load class names
	...
	if (infile.is_open())
	{
		...
		{
			_class_names.emplace_back(line);
		}
		infile.close();
	}
	else
...
/*	get class names		*/
std::vector<std::string> Detector::GetClassNames()
{
	return _class_names;
}

/*	loacal video stream.	*/
std::string Detector::gstreamer_pipeline(int capture_width, int capture_height, int display_width, int display_height, int framerate, int flip_method)
{
	// std::string pipeline = gstreamer_pipeline(CAPTURE_WIDTH, CAPTURE_HEIGHT, DISPLAY_WIDTH, DISPLAY_HEIGHT, FRAME_RATE, 0);
